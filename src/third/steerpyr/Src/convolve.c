...
/* --------------------------------------------------------------------
  ...
  specially computed edge-handling filters (see edges.c).  
  The origin of the filter is assumed to be (x_dim/2, y_dim/2).
  Subsampling is done on a rectangular grid specified by the grid 
  parameters.  
...
internal_expand(image,x_dim,y_dim,filt,temp,x_fdim,y_fdim,
				xgrid_start,xgrid_step,ygrid_start,ygrid_step,result,edges)
